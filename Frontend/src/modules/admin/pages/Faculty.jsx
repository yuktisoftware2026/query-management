...
import { getAllFaculty, createFaculty, deleteFaculty } from '@/modules/admin/api/adminApi'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-primary">Faculty Members</h2>
          <p className="text-text-secondary mt-1">Manage all faculty and teaching staff</p>
        </div>
      ...
      {loading ? <LoadingState message="Loading faculty..." />
        : error ? <div className="bg-danger/10 border border-danger text-danger p-4 rounded-lg"><p>{error}</p></div>
        : faculty.length === 0 ? <EmptyState icon={Users2} title="No faculty found" description="Add your first faculty member." />
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Add Faculty Member">
        <form onSubmit={handleCreate} className="space-y-4">
          {[
            { name: 'name', label: 'Full Name', placeholder: 'e.g. Dr. Yukti', type: 'text' },
            { name: 'email', label: 'Email', placeholder: 'faculty@example.com', type: 'email' },
            { name: 'phone', label: 'Phone', placeholder: '9876543210', type: 'text' },
            { name: 'specialization', label: 'Specialization', placeholder: 'e.g. Java, Python, DSA', type: 'text' },
          ].map(f => (
            <div key={f.name}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? 'Creating...' : 'Create Faculty'}</button>
          </div>
        </form>
      </Modal>
    </div>
